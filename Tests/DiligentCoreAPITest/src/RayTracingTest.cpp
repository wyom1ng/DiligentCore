...
RAYTRACING_BUILD_AS_FLAGS TestBLASFlags(Uint32 TestId)
{
    switch (TestId)
    {
        // clang-format off
...
RAYTRACING_BUILD_AS_FLAGS TestTLASFlags(Uint32 TestId)
{
    switch (TestId)
    {
        // clang-format off
...
TEST_P(RT1, TriangleClosestHitShader)
{
    ...
    CreateBLAS(pDevice, pContext, &Triangle, 1, TestBLASFlags(TestId), pTempBLAS);
    ...
    CreateTLAS(pDevice, pContext, &Instance, 1, HitGroupStride, TestTLASFlags(TestId), pTempTLAS);
...
TEST_P(RT3, ProceduralIntersection)
{
    ...
    RefCntAutoPtr<IBottomLevelAS> pTempBLAS;
    CreateBLAS(pDevice, pContext, &Box, 1, TestBLASFlags(TestId), pTempBLAS);
...
TEST_P(RT4, MultiGeometry)
{
    ...
    RefCntAutoPtr<IBottomLevelAS> pTempBLAS;
    CreateBLAS(pDevice, pContext, Triangles, _countof(Triangles), TestBLASFlags(TestId), pTempBLAS);
    ...
    RefCntAutoPtr<ITopLevelAS> pTempTLAS;
    const Uint32               HitGroupStride = 1;
    CreateTLAS(pDevice, pContext, Instances, _countof(Instances), HitGroupStride, TestTLASFlags(TestId), pTempTLAS);
